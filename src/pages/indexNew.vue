<template>
  ...
  >
    ...
    <el-form-item :label="$t('orderTool.receiveToken')" prop="token_name">
      <el-input v-model="ruleForm.token_name"></el-input>
    </el-form-item>
    <el-form-item :label="$t('orderTool.merchantAddress')" prop="address">
  ...
  <sign-dialog :signMessage="true" v-model:showModal="showModal" v-model:sign-content="signContent" @result="getSignResult" />
</template>

<script lang="ts" setup>
import { reactive, ref } from 'vue'
import { USDT, ChecksSiteHref1 } from '@/const/index'
import { SubmitSignMessageOrder } from '@/api/index'
  ...
  await formEl.validate(async (valid, fields) => {
    if (valid) {
      try {
        ...
        const { code, data, message } = await SubmitSignMessageOrder(orderInfo)
        loading.value = false
        if (code == 0) {
          const params = new URLSearchParams()
          params.append('uuid', data.uuid)
          window.open(`${ChecksSiteHref1}/?${params.toString()}`, '_blank')
        }
        else {
...
const checkSignFields = function () {
  return new Promise(resolve => {
    let res: Array<any> = []
    let t = 0
    ruleFormRef.value.validateField(['out_order_no', 'address','token_name', 'pay_amount', 'notify'], (vaild: string) => {
      t++
      res.push(vaild)
      if (t === 5) {
