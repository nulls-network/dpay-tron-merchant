<template>
    ...
    >
        ...
        <el-form-item :label="$t('orderTool.notifyUrl')" prop="notify_url">
         <el-input v-model="ruleForm.notify_url" placeholder="localhost"></el-input>
        </el-form-item>
...
import { doWithdrawNew } from '@/api/index'
...
const checkSignFields = function () {
    return new Promise(resolve => {
        let res: Array<any> = []
        let t = 0
        ruleFormRef.value.validateField(['to', 'token', 'amount', 'fromChain', 'toChain', 'uuid', 'notify_url'], (vaild: string) => {
            t++
            res.push(vaild)
            if (t === 7) {
    ...
    await formEl.validate(async (valid, fields) => {
        if (valid) {
            try {
                ...
                const res = await doWithdrawNew(ruleForm)
                loading.value = false
                response.value = res
                if (res.code == 0) {
                }
